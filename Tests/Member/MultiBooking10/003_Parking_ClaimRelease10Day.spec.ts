...
test.beforeAll(async ({ sharedPage }) => {
  ...
  const user = await reusablePageClass.loginWithExcelAdminUser2("MultiBook_5", ZoneData.ZONE_SET1[ENV]);
  //const user =  await reusablePageClass.loginWithExcelUser("Meeting01", ZoneData.ZONE_SET1[ENV]);
  zone = user.ZONE;
  ...
  await reusablePageClass.loginWithExcelUser("MultiBook_5", ZoneData.ZONE_SET1[ENV]);
