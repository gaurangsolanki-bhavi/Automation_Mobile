...
test.beforeAll(async ({ sharedPage }) => {
  ...
  const user = await reusablePageClass.loginWithExcelAdminUser2("MultiBook_2", ZoneData.ZONE_SET1[ENV]);
  //const user =  await reusablePageClass.loginWithExcelUser("DeskUser", ZoneData.ZONE_SET1[ENV]);
  zone = user.ZONE;
  ...
  await reusablePageClass.loginWithExcelUser("MultiBook_2", ZoneData.ZONE_SET1[ENV]);
