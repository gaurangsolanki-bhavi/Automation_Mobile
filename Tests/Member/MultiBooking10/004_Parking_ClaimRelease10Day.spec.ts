...
test.beforeAll(async ({ sharedPage }) => {
  ...
  const user = await reusablePageClass.loginWithExcelAdminUser2("MultiBook_3", ZoneData.ZONE_SET1[ENV]);
 // const user =  await reusablePageClass.loginWithExcelUser("ValidUser2", ZoneData.ZONE_SET1[ENV]);
  ...
  await reusablePageClass.loginWithExcelUser("MultiBook_3", ZoneData.ZONE_SET1[ENV]);
