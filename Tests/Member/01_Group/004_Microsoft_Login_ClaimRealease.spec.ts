...
test.beforeAll(async ({ sharedPage }) => {
  ...
  const user = await groupOnePage.loginMicrosoftWithExcelUser("MSLoginUser",ZoneData.ZONE_SET1.PROD);
  zone = user.ZONE;
...
test.describe(`Verify Microsoft SSO Login Member book and release bookings`, () => {

  test(`Test_001: Ensures a member can Microsoft SSO log in and claim and release a parking spot on the current day`, async () => {
    try {
