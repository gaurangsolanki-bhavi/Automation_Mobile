...
test.beforeAll(async ({ sharedPage }) => {
  ...
  const user = await groupOnePage.loginOktaWithExcelUser( "OktaLoginUser",ZoneData.ZONE_SET1.PROD);
  zone = user.ZONE;
...
test.describe(`Verify OKTA SSO Login Member book and release bookings`, () => {


  test(`Test_001: Ensures a member can OKTA SSO log in and claim and release a parking spot on the current day`, async () => {
    try {
