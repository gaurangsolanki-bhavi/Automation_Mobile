...
test.beforeAll(async ({ sharedPage }) => {
  ...
  const user = await groupOnePage.loginGmailWithExcelUser( "GmailLoginUser",ZoneData.ZONE_SET1.PROD);
  zone = user.ZONE;
...
test.describe(`Verify Gmail SSO Login Member book and release bookings`, () => {


  test(`Test_001: Ensures a member can Gmail SSO log in and claim and release a parking spot on the current day`, async () => {
    try {
