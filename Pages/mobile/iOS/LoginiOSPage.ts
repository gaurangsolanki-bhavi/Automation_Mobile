...
class LoginActionsIOS extends Actions {
  ...
  async initializeAppAndLoginFromExcel(sheetName: string, filePath: string, maxRetries: number = 1): Promise<any> {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        const selector = '//XCUIElementTypeStaticText[contains(@name, "Hi")]';
  ...
  async loginWithExcelUser(sheetName: string, filePath: string): Promise<any> {
    ...
    await this.loginWithEmailIOS(this.email, this.password);
    return user;
  }

  async loginWithEmailIOS(email: string, password: string) {
    ...
    await this.waitForElementAndClick(await Locatore.btnDone);
  }
  ...
  async loginWithGmail(email: string, password: string): Promise<void> {
    ...
    await driver.pause(3000);
    console.log('Entering password');
    await this.actionClass.waitForElementAndClick(await Locatore.passwordGmailInput);
    await this.clearAndSendKey(await Locatore.passwordGmailInput, password);
    ...
    const notNow = (await Locatore.savePasswordPopup).isDisplayed();
    if (await notNow) {
      await this.waitForElementAndClick(await Locatore.savePasswordPopup);
    }
    console.log('Clicking login done button');
    await this.waitForElementAndClick(await Locatore.btnDone);
    console.log("--------------------------------------------------------------------");
  }
  ...
  async initializeAppAndMSLoginFromExcel(sheetName: string, filePath: string, maxRetries: number = 1): Promise<any> {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (loginBtn) {
          console.log('App launched. Proceeding with login...');
          user = await this.loginWithExcelMSUser(sheetName, filePath);
        }
        // else {
        //   await this.logoutUser();
        // }
        ...
        if (attempt < maxRetries) {
          console.log('Closing and retrying...');
          await driver.execute('mobile: terminateApp', {
            bundleId: 'ie.jemstone.ronspot.view.activities.SplashActivity'
          });
          await driver.pause(2000);
        }
        attempt++;
  ...
  async loginWithMS(email: string, password: string): Promise<void> {
    ...
    await this.actionClass.waitForElementAndClick(await Locatore.passwordMicrosoftInput);
    ...
    await browser.pause(5000);
    const staySignedInYes = (await Locatore.staySignedInbutton).isDisplayed();
    if (await staySignedInYes){
      await this.waitForElementAndClick(await Locatore.staySignedInbutton);
    }
    console.log('Clicking login done button');
    await this.waitForElementAndClick(await Locatore.btnDone);
    console.log("--------------------------------------------------------------------");
  ...
  async initializeAppAndOktaLoginFromExcel(sheetName: string, filePath: string, maxRetries: number = 1): Promise<any> {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (loginBtn) {
          console.log('App launched. Proceeding with login...');
          user = await this.loginWithExcelOktaUser(sheetName, filePath);
        }
        // else {
        //   await this.logoutUser();
        // }
  ...
  async initializeAppAndManualLoginFromExcel(sheetName: string, filePath: string, maxRetries: number = 1): Promise<any> {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (loginBtn) {
          console.log('App launched. Proceeding with login...');
          user = await this.loginWithExcelManualUser(sheetName, filePath);
        }
        // else {
        //   await this.logoutUser();
        // }
...
export default LoginActionsIOS;